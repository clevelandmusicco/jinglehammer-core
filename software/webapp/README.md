...
`index.html` is the editor for the controller's presets, banks, and channels.
It talks over USB-CDC using WebSerial, so it opens straight off disk with
nothing installed. See the [software README](../README.md) for the
controller itself and [`../firmware/`](../firmware/README.md) for the device
side.
...
No npm and no bundler; it is plain Python 3 stdlib, so it runs the same on
...
3. Edit each preset: name, plus its Program Change and Control Change
...
4. **Write + Save** sends all 8 banks and commits them to flash.
